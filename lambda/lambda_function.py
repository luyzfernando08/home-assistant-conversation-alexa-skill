...
alexa_speak_output = config.get("alexa_speak_output").strip().split(';')
alexa_speak_waiting = config.get("alexa_speak_waiting")
alexa_bye_bye = config.get("alexa_bye_bye").strip().split(';')

# Verificação de configuração
if not alexa_bye_bye or not home_assistant_url or not home_assistant_token or not home_assistant_agent_id or not alexa_speak_output or not alexa_speak_waiting:
    raise ValueError("alexa_bye_bye, alexa_speak_waiting, alexa_speak_output, home_assistant_url, home_assistant_token ou home_assistant_agent_id não configurados corretamente")
...
class LaunchRequestHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        msg = random.choice(alexa_speak_output)
...
class CancelOrStopIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        speak_output = random.choice(alexa_bye_bye)
...
class CatchAllExceptionHandler(AbstractExceptionHandler):
    ...
    def handle(self, handler_input, exception):
        ...
        speak_output = f"Desculpe, não consegui processar sua solicitação. {str(e)}"
        return handler_input.response_builder.speak(speak_output).ask(speak_output).response
